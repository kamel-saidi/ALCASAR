...
then
	# on supprimme le backup de la conf, et on restaure le fichier de conf par defaut, on peut faire des test si les fichiers existe avant
	rm -f /etc/postfix/main.cf.bak
	mv -f /etc/postfix/main.cf.origin  /etc/postfix/main.cf
	
	# on suprimme le dossier conteant la configuration du mail par lequel les mails sont envoyés dans le cas 3
	...
	# on peut désinstaller le SALS, s'il est installé conf 3, mais c'est un mécanisme d'authentification qui peut être utilisé,
	# il ne prend peu d'espèce disque, et si on le laisse ça évite de le télécharger pour un futur install, économisé le débit pour certains pays???
fi
